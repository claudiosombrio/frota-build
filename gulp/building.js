...
gulp.task('build-app', ['clean', 'inject-all', 'copy_version'], function () {
...
gulp.task('build-templates', ['clean'], function () {
  ...
  .pipe($.angularTemplatecache('templates.js', {module: 'main', root: 'main/'}))
  .pipe(gulp.dest(paths.appMain));
});
...
gulp.task('copy_version', function(){
  return gulp.src('app/versao.txt')
    .pipe(gulp.dest('www'));
})
